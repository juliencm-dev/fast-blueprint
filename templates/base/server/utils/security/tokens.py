...
from server.models import AccessTokenResponse, RefreshTokenData, RefreshTokenResponse, ValidationTokenData, AccessTokenData
from server.exceptions.auth import InvalidVerificationTokenException, TokenExpiredException, TokenNotFoundException, InvalidCredentialsException , InvalidRefreshTokenException
...
class TokenManager:
    ...
    def create_access_token(self, data: dict) -> AccessTokenResponse:
        ...
        to_encode = data.copy()
        to_encode.update({"exp": self._set_token_expiration(s.ACCESS_TOKEN_EXPIRE_MINUTES)})
    ...
    async def create_refresh_token(self, data: dict) -> RefreshTokenResponse:
        ...
        to_encode = data.copy()
        to_encode.update({"exp": self._set_token_expiration(s.REFRESH_TOKEN_EXPIRE_MINUTES)})
        to_encode.update({"jti": cuid()})

        refresh_token = RefreshTokenData(jti=to_encode["jti"], user_id=to_encode["sub"], device_id=to_encode["device_id"], expires_at=to_encode["exp"])
    ...
    async def create_validation_token(self, user_id: str, token_type: ValidationTokenType) -> str:
        """Create a validation token. Takes the user's id and the token type as arguments."""
        
        existing_token = await self._auth_dao.get_validation_token_by_user_id_and_type(user_id, token_type)
    ...
    async def verify_validation_token(self, token_str: str) -> Tuple[User, ValidationToken]:
    ...
    def _generate_validation_token(self, user_id: str, token_type:ValidationTokenType) -> ValidationTokenData:
        ...
        return ValidationTokenData(user_id=user_id, 
                                       token=cuid(), 
