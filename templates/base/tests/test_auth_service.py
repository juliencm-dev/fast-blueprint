...
async def test_login_success(
    ...
    mock_token_manager.create_refresh_token.return_value = (
        sample_refresh_token_login_response
    )
...
async def test_activate_account_success(
    ...
    assert (
        result["message"]
        == "Successfully verified email address. Your account is now activated."
    )
...
async def test_activate_account_expired_token(
    ...
    assert (
        result["message"]
        == "Verification link as expired. A new link has been sent to your email address."
    )
...
async def test_request_password_reset_success(
    mock_auth_service, sample_refresh_token, mock_email_service, mock_token_manager
):
...
async def test_reset_password_success(
    ...
    result = await mock_auth_service.reset_password(
        "valid_token",
        PasswordResetRequest(password="new_password", confirm_password="new_password"),
    )

    # Assert expected behavior
    assert (
        result["message"]
        == "Your password has been successfully reset. Your can now login using your new password."
    )
...
async def test_reset_password_password_mismatch(
    ...
    mock_token_manager.verify_validation_token.return_value = (
        ...
        MagicMock(
            user_id="123",
            token="valid_token",
            expires_at=nowutc() + timedelta(minutes=10),
    ...
    result = await mock_auth_service.reset_password(
        "valid_token",
        PasswordResetRequest(password="new_password", confirm_password="new_psord"),
    )
...
async def test_reset_password_expired_token(
    ...
    assert (
        result["message"]
        == "Verification link as expired. If you did not request a password reset, please ignore this warning. Otherwise, please request a new password reset link."
    )
