...
@pytest.mark.asyncio
async def test_get_users_as_admin(app,test_client, mock_user_service, admin_user_override, mock_access_token):
    try:
        ...
        users = [
            UserResponse(first_name="John", last_name="Doe", email="john.doe@example.com", role=UserRole.USER),
            UserResponse(first_name="Jane", last_name="Doe", email="jane.doe@example.com", role=UserRole.USER),
            UserResponse(first_name="Bob", last_name="Doe", email="bob.doe@example.com", role=UserRole.USER),
        ]
        mock_user_service.get_users.return_value = [User(**user.model_dump()) for user in users]
...
@pytest.mark.asyncio
async def test_get_users(app, test_client, mock_user_service, mock_current_user, mock_access_token):
    current_user_instance = mock_current_user(email="user@example.com", role=UserRole.USER)
    ...
    try:
        ...
        assert response_json["message"] == "Only admins are allowed to access this route."
...
async def test_get_current_user(app,test_client, mock_current_user, mock_access_token):
    try:
        ...
        dumped_user["verified"] = current_user_instance.verified.isoformat().replace("+00:00", "Z")
...
@pytest.mark.asyncio
async def test_get_user_by_id(app, test_client, mock_user_service, mock_current_user, mock_access_token):
    try:
        ...
        if user_response.verified:
            expected_response["verified"] = user_response.verified.isoformat().replace("+00:00", "Z")
...
@pytest.mark.asyncio
async def test_update_user(app, test_client, mock_user_service, mock_current_user, mock_access_token):
    try:
        # Create a mock current user
        current_user_instance = mock_current_user(email="admin@example.com", role=UserRole.ADMIN)
        app.dependency_overrides[get_current_active_user] = lambda: current_user_instance
        ...
        response = test_client.patch(f"{s.API_PREFIX}/users/123", headers=headers, json=payload)
...
@pytest.mark.asyncio
async def test_delete_user(app,test_client, mock_user_service, mock_current_user, mock_access_token):
    try:
        # Create a mock current user
        current_user = mock_current_user(email="admin@example.com", role=UserRole.ADMIN)
        app.dependency_overrides[get_current_active_user] = lambda: current_user
